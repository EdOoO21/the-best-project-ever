        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Установить оповещение", callback_data="set_alert"
                ),
...
def subscribe_button(route_id: int):
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Подписаться на маршрут", callback_data=f"subscribe_{route_id}"
                )
            ]
        ]
    )
...
def list_user_subscriptions(user_id: int):
    ...
    """
    subs = (
        session.query(Subscription, Route)
    ...
    for sub, route in subs:
        from_st = (
            session.query(Station).filter_by(station_id=route.from_station_id).first()
        )
        ...
        date_str = (
            route.from_date.strftime("%d.%m.%Y") if route.from_date else "неизвестно"
        )
...
async def cmd_start(message: types.Message):
    ...
    if user and user.status == UserStatus.banned:
        ...
        logger.info(
            f"Блокированный пользователь {user_id} попытался использовать бота."
        )
        return
    ...
    if user_id not in registered:
        ...
        welcome_text = f"С возвращением, {message.from_user.first_name}! Что вы хотите сделать сегодня?"
...
async def cb_set_alert(callback_query: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        user_id, "Введите точное наименование города отбытия с маленькой буквы:"
    )
...
async def process_origin(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введите точное наименование города назначения с маленькой буквы:"
    )
...
async def process_date(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Выберите класс билета для оповещения:", reply_markup=ticket_options_keyboard()
    )
...
async def cb_delete_alert(callback_query: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        user_id, "Введите ID оповещения (маршрута), которое хотите удалить:"
    )
...
async def process_delete_alert_id(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Оповещение успешно удалено.", reply_markup=main_menu_keyboard()
    )
    await state.clear()
...
async def process_ticket_class(callback_query: CallbackQuery, state: FSMContext):
    ...
    if not all(k in data for k in ("origin", "destination", "date")):
        await bot.send_message(
            user_id, "Произошла ошибка. Пожалуйста, начните заново /start."
        )
        await state.clear()
        return
    ...
    if not isinstance(result_data, dict):
        await bot.send_message(
            user_id, "Произошла ошибка при получении данных о маршрутах."
        )
        await state.clear()
        return
...
async def cb_subscribe_route(callback_query: CallbackQuery):
    ...
    await bot.send_message(
        user_id,
        f"Вы подписались на маршрут ID {route_id}!",
        reply_markup=main_menu_keyboard(),
    )
...
async def subscribe_route(message: Message):
    ...
    if len(args) < 2:
        await message.answer(
            "Неверный формат команды. Используйте /subscribe <route_id>"
        )
        return
...
async def unsubscribe_route(message: Message):
    ...
    if len(args) < 2:
        await message.answer(
            "Неверный формат команды. Используйте /unsubscribe <route_id>"
        )
        return
