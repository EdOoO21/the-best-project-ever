from src.models import City, Station, Route, User, Subscription, Ticket, UserStatus, Base
...
def add_station(city_id: int, station_id: int, station_name: str):
    """загружаем станцию"""
    new_station = Station(city_id=city_id, station_name=station_name, station_id=station_id)
    session.add(new_station)
    session.commit()

def add_route(from_station_id: int, to_station_id: int, from_date: str, to_date: str, train_no: str):
    """добавляем новый маршрут"""
    new_route = Route(from_station_id=from_station_id, to_station_id=to_station_id, from_date=from_date, to_date=to_date, train_no=train_no)
...
def delete_route(route_id: int):
    ...
    if route:
        ...
        session.query(Subscription).filter_by(route_id=route_id).delete(synchronize_session=False)
...
def update_user(user_id: int, new_status: str):
    ...
    if user:
        ...
        if new_status == "banned":
            ...
            session.query(Subscription).filter_by(user_id=user_id).delete(synchronize_session=False)
        session.commit()
...
def delete_subscription(user_id: int, route_id: int):
    ...
    subscription = session.query(Subscription).filter_by(user_id=user_id, route_id=route_id).first()
    if subscription:
